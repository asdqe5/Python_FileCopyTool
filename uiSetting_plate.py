...
def initUIFunc(self):
    ...
    self.ui.plate_project_comboBox.clear()
    ...
    self.ui.plate_project_comboBox.addItems([""] + newProjectList)

    self.ui.plate_pathtocopy_lineEdit.clear()
    self.ui.plate_pathtocopy_lineEdit.setReadOnly(True)
    self.ui.plate_excelFile_lineEdit.clear()
    self.ui.plate_excelFile_lineEdit.setReadOnly(True)

    # 시퀀스 샷 테이블 설정
    initTableFunc(self)
    self.ui.plate_fileCopy_tableWidget.setColumnCount(len(self.plate_column_headers))
    self.ui.plate_fileCopy_tableWidget.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
    self.ui.plate_fileCopy_tableWidget.setHorizontalHeaderLabels(self.plate_column_headers)
    self.ui.plate_fileCopy_tableWidget.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeToContents)
    self.ui.plate_fileCopy_tableWidget.horizontalHeader().setMinimumSectionSize(100)

    # 초기에는 엑셀로 설정이 되어있다고 생각
    self.ui.plate_excel_radioButton.setChecked(True)
...
def initUIForShotgunFunc(self):
    ...
    '''
    self.ui.plate_project_comboBox.setCurrentIndex(0)
    self.ui.plate_pathtocopy_lineEdit.clear()
    self.ui.plate_vendor_comboBox.setEnabled(True)
    self.ui.plate_vendor_comboBox.clear()
    ...
    initTableFunc(self)
    self.ui.plate_copy_pushButton.setEnabled(True)
...
def initTableFunc(self):
    ...
    함수이다.
    '''
    self.ui.plate_fileCopy_tableWidget.clearContents()
    while (self.ui.plate_fileCopy_tableWidget.rowCount() > 0):
        self.ui.plate_fileCopy_tableWidget.removeRow(0)
    self.ui.plate_fileCopy_tableWidget.setRowCount(0)
...
def openBrowserFunc(self):
    ...
    self.ui.plate_pathtocopy_lineEdit.setText(folderPath)
...
def openBrowserForExcelFunc(self):
    ...
    self.ui.plate_excelFile_lineEdit.setText(filePath[0])
...
def setVendorComboBoxFunc(self):
    ...
    self.ui.plate_vendor_comboBox.clear()
    if self.ui.plate_project_comboBox.currentText() == "":
        return
    ...
    self.ui.plate_vendor_comboBox.addItems([""] + vendorList)
...
def shotcodeSettingByExcelFunc(self):
    ...
    for row in range(worksheet.nrows):
        ...
        rowPosition = self.ui.plate_fileCopy_tableWidget.rowCount()
        self.ui.plate_fileCopy_tableWidget.insertRow(rowPosition)

        # 샷코드 추가
        item = QtWidgets.QTableWidgetItem(shotCode)
        item.setTextAlignment(QtCore.Qt.AlignVCenter | QtCore.Qt.AlignHCenter)
        self.ui.plate_fileCopy_tableWidget.setItem(rowPosition, self.plate_column_idx_lookup["ShotCode"], item)
...
def shotcodeSettingByVendorFunc(self):
    ...
    if self.ui.plate_vendor_comboBox.currentText() == "":
        dial = QtWidgets.QMessageBox()
        dial.setWindowTitle("Error")
